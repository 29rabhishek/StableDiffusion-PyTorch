...
def train(args):
    #accelerator init
    accelerator = Accelerator(gradient_accumulation_steps=2, log_with="wandb")

    accelerator.init_trackers("Stable-Uncond-Diffusion")
    ...
    if not im_dataset.use_latents:
        ...
        vae = VAE(im_channels=dataset_config['im_channels'],
                    model_config=autoencoder_model_config).to(accelerator.device)
        vae.eval()
        # Load vae if found
    ...
    for epoch_idx in range(num_epochs):
        ...
        for im in data_loader:
            ...
            # loss.backward()
            optimizer.step()
        accelerator.log('Finished epoch:{} | Loss : {:.4f}'.format(epoch_idx + 1,np.mean(losses)))
